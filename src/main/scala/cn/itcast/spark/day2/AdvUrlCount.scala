...
object AdvUrlCount {
  ...
  def main(args: Array[String]) {
    ...
    val rdd1 = sc.textFile("c://itcast.log").map(line => {
      val f = line.split("\t")
      (f(1), 1)
      // (http://java.itcast.cn/java/course/javaee.shtml,  1)
      // (http://java.itcast.cn/java/course/hadoop.shtml , 1)
    })
    ...
    val rdd3 = rdd2.map(t => {
      ...
      (host, url, t._2)   //(http://java.itcast.cn/java/course/javaee.shtml  ,java.itcast.cn,3)
    })
