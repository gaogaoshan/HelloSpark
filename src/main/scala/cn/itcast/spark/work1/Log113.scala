...
//    val parquetFile=sqlContext.read.parquet("hdfs:/logs/logformat/113/dt=2017040300")
//    val selectDf=parquetFile.select("ssid","suv","url","referdomain","svn").cache()


    val aaa = selectDf.filter(selectDf("ref_url").contains("17173.com") && selectDf("url").contains("yeyou.com")).select("ssid").distinct()
    ...
    val fromYeyou_SidRdd: RDD[((Any, Any), (Any, String))] = bbb.join(aaa, aaa("ssid") === bbb("ssid")).rdd.map(row => {
      ((row(0), row(1)), (row(2),  row(5).toString))//(ssid,uv),(ads_code,svn)
    })
...
//    PV UV join  根据PV排序=====（adsCode,第几次）,（PV，UV）==============================================================================================================
    ...
    //翻译模块名称
      ...
      val mapRusult= joinRusult.map(x=>{
        x._1._2+","+adsCodeNameBroadCast.getOrElse(x._1._1,x._1._1)+","+x._2._1+","+x._2._2
      })
