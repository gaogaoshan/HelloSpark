...
//    1.获取访问173首页的用户    moduleRdd.rdd.getNumPartitions
    ...
    val month=List("201705")
    val trafficSql="select suv as uv,ddate from traffic where ddate in('20170510') and domain='www.17173.com'  group by suv, ddate";
    val moduleSql="select uv,ddate,isnew,ts,url,ads_code,ref_type FROM module WHERE ddate in ('20170510') "
    val guanggaoSql="""select suv as uv,ddate  from click
...
//    val clickRdd=sparkSession.sql(guanggaoSql)
...
//     delCode.saveAsTextFile("hdfs:/tmp/hugsh/laoqu/delCode")
    ...
    val filterRdd= joinRdd.filter(f=>{!dCode.contains(f._2._3)})//count=day5=5244003 filterRdd.saveAsTextFile("hdfs:/tmp/hugsh/laoqu/tmpModule5-filterRdd")
...
//    val bigthan2Month=path_MonthCount.filter(x=>x._2.size>4).cache()
    ...
    path_Count6.saveAsTextFile("hdfs:/tmp/hugsh/laoqu/tmpModule5-pathString")
