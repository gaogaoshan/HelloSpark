...
// =======================================================================================================================================
    ...
    val _113parquetFile=sqlContext.read.parquet("hdfs:/logs/logapi/113/*20170307*").createOrReplaceTempView("113_log")
    val _118parquetFile=sqlContext.read.parquet("hdfs:/logs/logapi/118/*20170307*").createOrReplaceTempView("118_log")
...
//    2.用118全部数据和1的结果关联得到 满足条件用户的的点击轨迹
//    3.根据UV分组 并按时间排序  每个分组内取前3次点击
...
//             and  click_url='http://newgame.17173.com/game-info-1000219.html'  """.stripMargin.replaceAll("\n","");
  val ssidSql="""select ssid,first(ads_code)  from 113_log
             where url like '%www.17173.com%'
             and  ads_code in('3a9b3f036a2f571a23e1c499d1477791','6273154c808759048e3200c51cbb44b1','9c9f1366edbae758a3bb56eca4388b64')
             and  click_url='http://newgame.17173.com/game-info-1000219.html'
             group by ssid """.stripMargin.replaceAll("\n","");
    val uvSet= sqlContext.sql(ssidSql)


   //2.用全部数据和1的结果关联得到 满足条件用户的的点击轨迹
   // join 后=ssid,cur_url,u.ts,adsCode     我的顺序=ssid，cur_url，adsCode，u，ts       allData.count()=3917661
    val allData=sqlContext.sql("select ssid,cur_url,u,ts from 118_log  ")
    val joinRdd= allData.join(uvSet, "ssid").rdd.map(r => (r(0), (r(1).toString, r(4).toString, r(2).toString, r(3).toString))).cache()



    //3.根据UV分组 并按时间ts排序  取冒险岛后的4次url (uv,adsCode,urls)
    val take4Rdd: RDD[(Any, String, String)] = joinRdd.groupByKey().flatMap(x => {
      val ssid = x._1
      val clickList = x._2 //url,ads_code,click_url,ts
    ...
    resultRdd.saveAsTextFile("hdfs:/tmp/hugsh/laoqu/mxd5")



    val selectData11=sqlContext.sql("select ssid,cur_url,u,ts from 118_log   order by ts")
    selectData11.rdd.saveAsTextFile("hdfs:/tmp/hugsh/laoqu/t4")
